...
playlists$ArtistToken<-apply(t,MARGIN=1,FUN=paste,collapse=" ") #collapse first numwords words into one name

# There are a dozen ways Andy Breckman can misspell "Bruce Springsteen."
playlists<- playlists %>%   
  mutate(ArtistToken=replace(ArtistToken,
                             str_detect(ArtistToken,'Bruce Sp'),
                             "Springsteen"))
...
print('Stripping repeated closing songs that would distort analysis.')
songs_to_strip<-playlists %>%
...
print("Stripping Songs where only one DJ plays it - over and over")
TOLERANCE <- 0.987 
